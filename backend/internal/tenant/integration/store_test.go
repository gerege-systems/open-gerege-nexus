...
//
//	INTEGRATION_TEST_DATABASE_URL=postgres://... go test ./internal/tenant/integration/...
//
...
func TestTheStoredSecretNeverLeavesThePackage(t *testing.T) {
	...
	if err := pool.QueryRow(ctx,
		`SELECT secret_ciphertext FROM tenant.integrations WHERE id = $1`, created.ID).Scan(&raw); err != nil {
