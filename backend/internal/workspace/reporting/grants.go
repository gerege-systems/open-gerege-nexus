...
func ActiveGrants(ctx context.Context, db Queryer, granteeTenantID, reportKey string) ([]Grant, error) {
	rows, err := db.Query(nexus.WithoutWorkspace(ctx), `
...
func (e *Engine) auditBothSides(ctx context.Context, grant Grant, granteeTenantID,
	...
	audit.Record(nexus.WithWorkspaceID(ctx, granteeTenantID), granteeTenantID, actorUserID,
	...
	audit.Record(nexus.WithWorkspaceID(ctx, grant.GrantorTenantID), grant.GrantorTenantID, "",
...
func ListGrants(ctx context.Context, db Queryer, tenantID string) ([]GrantRow, error) {
	rows, err := db.Query(nexus.WithWorkspaceID(ctx, tenantID), `
