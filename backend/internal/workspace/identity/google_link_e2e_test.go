...
func (f *linkFixture) newSignedInPerson(t *testing.T) (userID, token string) {
	...
	if _, err := f.pool.Exec(ctx,
		`INSERT INTO workspace.memberships(tenant_id, user_id) VALUES($1,$2) ON CONFLICT DO NOTHING`,
...
func TestALinkWithoutASessionLinksNothing(t *testing.T) {
	...
	if _, err := f.pool.Exec(context.Background(),
		`UPDATE workspace.sessions SET revoked_at = NOW() WHERE user_id = $1`, f.userID); err != nil {
