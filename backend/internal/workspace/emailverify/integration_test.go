...
//
//	EMAILVERIFY_TEST_DATABASE_URL=postgres://... go test ./internal/workspace/emailverify/...
...
func TestAnExpiredRequestIsRefusedAndSweptUp(t *testing.T) {
	...
	if _, err := f.pool.Exec(ctx,
		`UPDATE workspace.email_verifications SET expires_at = NOW() - INTERVAL '1 minute' WHERE id = $1`,
		sent.ID); err != nil {
	...
	if err := f.pool.QueryRow(ctx,
		`SELECT status FROM workspace.email_verifications WHERE id = $1`, sent.ID).Scan(&status); err != nil {
...
func TestARefusedRequestLeavesNoRowBehind(t *testing.T) {
	...
	var count int
	if err := f.pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM workspace.email_verifications WHERE tenant_id = $1 AND email = $2`,
