...
func seedMember(t *testing.T, pool *pgxpool.Pool) (tenantID, userID, roleID, permission string) {
	...
	if err := pool.QueryRow(ctx,
		`INSERT INTO workspace.roles (tenant_id, code, name) VALUES ($1,'cachetest','Cache Test') RETURNING id::text`,
...
func TestRevokingAPermissionTakesEffectOnceTheTenantIsInvalidated(t *testing.T) {
	...
	revoke := func() {
		if _, err := pool.Exec(ctx,
			`DELETE FROM workspace.role_permissions WHERE role_id=$1`, roleID); err != nil {
			t.Fatal(err)
		}
	}
