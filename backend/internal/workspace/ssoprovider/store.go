...
func (s *Store) GetClient(ctx context.Context, clientID string) (*Client, error) {
	return scanClient(s.db.QueryRow(ctx,
		`SELECT `+clientColumns+` FROM workspace.oauth2_clients WHERE client_id = $1`, clientID))
}
...
func (s *Store) GetTenantClient(ctx context.Context, tenantID, clientID string) (*Client, error) {
	return scanClient(s.db.QueryRow(ctx,
		`SELECT `+clientColumns+` FROM workspace.oauth2_clients WHERE tenant_id = $1 AND client_id = $2`,
		tenantID, clientID))
}
...
func (s *Store) ListClients(ctx context.Context, tenantID string) ([]*Client, error) {
	rows, err := s.db.Query(ctx,
		`SELECT `+clientColumns+` FROM workspace.oauth2_clients WHERE tenant_id = $1 ORDER BY created_at DESC`,
		tenantID)
	if err != nil {
		return nil, err
	}
...
func (s *Store) UpdateClient(ctx context.Context, tenantID string, c *Client) (*Client, error) {
	return scanClient(s.db.QueryRow(ctx, `
		UPDATE workspace.oauth2_clients
...
func (s *Store) DeleteClient(ctx context.Context, tenantID, clientID string) error {
	tag, err := s.db.Exec(ctx,
		`DELETE FROM workspace.oauth2_clients WHERE tenant_id = $1 AND client_id = $2`, tenantID, clientID)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
...
func (s *Store) VerifyClientSecret(ctx context.Context, clientID, secret string) (*Client, error) {
	...
	err := s.db.QueryRow(ctx,
		`SELECT client_secret_hash FROM workspace.oauth2_clients WHERE client_id = $1 AND NOT disabled`,
...
func (s *Store) TouchClient(ctx context.Context, clientID string) {
	_, _ = s.db.Exec(ctx, `UPDATE workspace.oauth2_clients SET last_used_at = NOW() WHERE client_id = $1`, clientID)
}
...
func (s *Store) SaveAuthCode(ctx context.Context, ac *AuthCode) error {
	_, err := s.db.Exec(ctx, `
		INSERT INTO workspace.oauth2_authorization_codes
...
func (s *Store) ConsumeAuthCode(ctx context.Context, codeHash string) (*AuthCode, error) {
	var ac AuthCode
	err := s.db.QueryRow(ctx, `
		UPDATE workspace.oauth2_authorization_codes
		   SET consumed_at = NOW()
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		if lookupErr := s.db.QueryRow(ctx,
			`SELECT consumed_at IS NOT NULL FROM workspace.oauth2_authorization_codes WHERE code_hash = $1`,
...
func (s *Store) SaveToken(ctx context.Context, t *Token) (*Token, error) {
	err := s.db.QueryRow(ctx, `
		INSERT INTO workspace.oauth2_tokens
...
func (s *Store) RevokeToken(ctx context.Context, token string) error {
	_, err := s.db.Exec(ctx,
		`UPDATE workspace.oauth2_tokens SET revoked_at = NOW() WHERE token_hash = $1 AND revoked_at IS NULL`,
		hashSecret(token))
	return err
}
...
func (s *Store) RevokeFamily(ctx context.Context, tokenID string) error {
	_, err := s.db.Exec(ctx, `
		WITH RECURSIVE family AS (
			SELECT id FROM workspace.oauth2_tokens WHERE id = $1
			UNION ALL
			SELECT t.id FROM workspace.oauth2_tokens t JOIN family f ON t.parent_id = f.id
		)
		UPDATE workspace.oauth2_tokens SET revoked_at = NOW()
...
func (s *Store) RevokeByAuthCode(ctx context.Context, codeHash string) error {
	_, err := s.db.Exec(ctx,
		`UPDATE workspace.oauth2_tokens SET revoked_at = NOW() WHERE auth_code_hash = $1 AND revoked_at IS NULL`,
		codeHash)
	return err
}
...
func (s *Store) DeleteExpired(ctx context.Context) (int64, error) {
	codes, err := s.db.Exec(ctx,
		`DELETE FROM workspace.oauth2_authorization_codes WHERE expires_at < NOW() - INTERVAL '1 hour'`)
	if err != nil {
		return 0, err
	}
	...
	tokens, err := s.db.Exec(ctx,
		`DELETE FROM workspace.oauth2_tokens
...
func (s *Store) GetConsent(ctx context.Context, userID, clientID string) ([]string, error) {
	var scopes []string
	err := s.db.QueryRow(ctx,
		`SELECT scopes FROM workspace.oauth2_consents WHERE user_id = $1 AND client_id = $2`,
...
func (s *Store) SaveConsent(ctx context.Context, tenantID, userID, clientID string, scopes []string) error {
	_, err := s.db.Exec(ctx, `
		INSERT INTO workspace.oauth2_consents (tenant_id, user_id, client_id, scopes)
		VALUES ($1,$2,$3,$4)
...
func (s *Store) RevokeConsent(ctx context.Context, userID, clientID string) error {
	if _, err := s.db.Exec(ctx,
		`DELETE FROM workspace.oauth2_consents WHERE user_id = $1 AND client_id = $2`, userID, clientID); err != nil {
		return err
	}
	_, err := s.db.Exec(ctx,
		`UPDATE workspace.oauth2_tokens SET revoked_at = NOW()
...
func (s *Store) ClientActivityByTenant(ctx context.Context, tenantID string) ([]ClientActivity, error) {
	rows, err := s.db.Query(ctx, `
		SELECT c.client_id, c.client_name, c.client_type, c.disabled, c.last_used_at,
		       ...
		       (SELECT count(*) FROM workspace.oauth2_consents oc WHERE oc.client_id = c.client_id)
		  FROM workspace.oauth2_clients c
		  LEFT JOIN workspace.oauth2_tokens t ON t.client_id = c.client_id
		 WHERE c.tenant_id = $1
...
func (s *Store) ConsentsByTenant(ctx context.Context, tenantID string, limit int) ([]ConsentRecord, error) {
	rows, err := s.db.Query(ctx, `
		SELECT c.client_id, c.client_name, u.id::text, u.email, u.name, oc.scopes, oc.granted_at
		  FROM workspace.oauth2_consents oc
		  JOIN workspace.oauth2_clients c ON c.client_id = oc.client_id
...
func (s *Store) RevokeClientTokens(ctx context.Context, tenantID, clientID string) (int64, error) {
	tag, err := s.db.Exec(ctx, `
		UPDATE workspace.oauth2_tokens t SET revoked_at = NOW()
		  FROM workspace.oauth2_clients c
...
func (s *Store) WithdrawConsent(ctx context.Context, tenantID, clientID, userID string) error {
	tag, err := s.db.Exec(ctx, `
		DELETE FROM workspace.oauth2_consents oc
		 WHERE oc.client_id = $2 AND oc.user_id = $3
		   AND EXISTS (SELECT 1 FROM workspace.oauth2_clients c
