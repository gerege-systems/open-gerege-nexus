...
func seedMember(t *testing.T, pool *pgxpool.Pool) (userID, tenantID string) {
	...
	if _, err := pool.Exec(ctx,
		`INSERT INTO workspace.memberships (tenant_id, user_id) VALUES ($1,$2)`, tenantID, userID); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
...
func TestASessionLeftAloneStopsWorking(t *testing.T) {
	...
	if _, err := pool.Exec(ctx,
		`UPDATE workspace.sessions SET last_seen_at = NOW() - INTERVAL '31 minutes' WHERE user_id=$1`,
		userID); err != nil {
		t.Fatal(err)
	}
	if _, err := store.Resolve(ctx, token); err == nil {
	...
	if err := pool.QueryRow(ctx,
		`SELECT expires_at > NOW() FROM workspace.sessions WHERE user_id=$1`, userID).Scan(&expiresInFuture); err != nil {
		t.Fatal(err)
	}
	if !expiresInFuture {
...
func TestUsingASessionKeepsItAlive(t *testing.T) {
	...
	if _, err := pool.Exec(ctx,
		`UPDATE workspace.sessions SET last_seen_at = NOW() - INTERVAL '20 minutes' WHERE user_id=$1`,
		userID); err != nil {
		t.Fatal(err)
	}
	if _, err := store.Resolve(ctx, token); err != nil {
	...
	if err := pool.QueryRow(ctx,
		`SELECT EXTRACT(EPOCH FROM (NOW() - last_seen_at)) FROM workspace.sessions WHERE user_id=$1`,
...
func TestResolvingDoesNotWriteOnEveryRequest(t *testing.T) {
	...
	var first time.Time
	if err := pool.QueryRow(ctx, `SELECT last_seen_at FROM workspace.sessions WHERE user_id=$1`, userID).Scan(&first); err != nil {
		t.Fatal(err)
	}
	for range 5 {
		if _, err := store.Resolve(ctx, token); err != nil {
	...
	var after time.Time
	if err := pool.QueryRow(ctx, `SELECT last_seen_at FROM workspace.sessions WHERE user_id=$1`, userID).Scan(&after); err != nil {
		t.Fatal(err)
	}
...
func TestTheIdleTimeoutCanBeTurnedOff(t *testing.T) {
	...
	if _, err := pool.Exec(ctx,
		`UPDATE workspace.sessions SET last_seen_at = NOW() - INTERVAL '30 days' WHERE user_id=$1`,
		userID); err != nil {
		t.Fatal(err)
	}
	if _, err := store.Resolve(ctx, token); err != nil {
