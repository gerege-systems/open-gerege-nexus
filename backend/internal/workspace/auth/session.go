...
func (s *SessionStore) Resolve(ctx context.Context, token string) (UserClaims, error) {
	...
	err := s.db.QueryRow(ctx,
		`WITH live AS (
		    SELECT id FROM workspace.sessions
		     WHERE token_hash = $1
		       AND revoked_at IS NULL
		       AND expires_at > NOW()
		       AND ($2::timestamptz IS NULL OR last_seen_at > $2)
		), touched AS (
		    UPDATE workspace.sessions SET last_seen_at = NOW()
...
func (s *SessionStore) Revoke(ctx context.Context, token string) error {
	...
	_, err := s.db.Exec(ctx,
		`UPDATE workspace.sessions SET revoked_at = NOW() WHERE token_hash = $1 AND revoked_at IS NULL`,
		hashToken(token))
	return err
}
...
func (s *SessionStore) SwitchTenant(ctx context.Context, token, tenantID string) (string, time.Time, error) {
	...
	var userID string
	err = tx.QueryRow(ctx,
		`SELECT user_id::text FROM workspace.sessions
	...
	if err := tx.QueryRow(ctx,
		`INSERT INTO workspace.sessions (token_hash, user_id, tenant_id, auth_method, user_agent, ip_address, expires_at)
		 SELECT $2, user_id, $3, auth_method, user_agent, ip_address, expires_at
		   FROM workspace.sessions WHERE token_hash = $1
...
func (s *SessionStore) SetActiveTenants(ctx context.Context, token string, tenantIDs []string) ([]string, error) {
	...
	if err := s.db.QueryRow(ctx,
		`SELECT tenant_id::text FROM workspace.sessions
		  WHERE token_hash = $1 AND revoked_at IS NULL AND expires_at > NOW()`,
	...
	if _, err := s.db.Exec(ctx,
		`UPDATE workspace.sessions SET allowed_tenant_ids = $2::uuid[] WHERE token_hash = $1`,
...
func (s *SessionStore) DeleteExpired(ctx context.Context) (int64, error) {
	tag, err := s.db.Exec(ctx,
		`DELETE FROM workspace.sessions WHERE expires_at < NOW() - INTERVAL '7 days'`)
	if err != nil {
		return 0, err
	}
	return tag.RowsAffected(), nil
