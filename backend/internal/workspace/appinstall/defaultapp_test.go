...
func TestEveryTenantGetsTheDefaultAppWithoutAnybodyInstallingIt(t *testing.T) {
	...
	if err := pool.QueryRow(ctx,
		`SELECT installed_version FROM workspace.app_installations WHERE tenant_id = $1 AND app_id = $2`,
	...
	var count int
	if err := pool.QueryRow(ctx,
		`SELECT count(*) FROM workspace.app_installations WHERE tenant_id = $1 AND app_id = $2`,
...
func TestTheDefaultAppCanBeRemovedAndStaysRemovedWithoutLosingData(t *testing.T) {
	...
	var status string
	if err := pool.QueryRow(ctx,
		`SELECT enabled, status FROM workspace.app_installations WHERE tenant_id = $1 AND app_id = $2`,
