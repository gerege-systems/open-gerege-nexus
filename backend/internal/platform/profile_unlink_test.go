...
func unlinkUser(t *testing.T, pool *pgxpool.Pool) string {
	...
	if err := pool.QueryRow(ctx,
		`INSERT INTO users(email, password_hash, name) VALUES($1, 'x', 'unlink probe') RETURNING id::text`,
...
func addSSOIdentity(t *testing.T, pool *pgxpool.Pool, userID, issuer, subject string) {
	...
	if _, err := pool.Exec(context.Background(),
		`INSERT INTO user_sso_identities(user_id, issuer, subject, email, name, claims)
...
func TestRemovingOneOfTwoLeavesTheOtherPinned(t *testing.T) {
	...
	if _, err := pool.Exec(ctx,
		`DELETE FROM user_sso_identities WHERE user_id=$1 AND subject=$2`, userID, subject); err != nil {
