...
func (s *Server) requireLocalLogin(next http.HandlerFunc) http.HandlerFunc {
...
func (s *Server) ssoStartURL() string {
...
func (s *Server) touchSSOIdentity(ctx context.Context, issuer string, identity *ssoclient.Identity) {
	if _, err := s.db.Exec(ctx,
