...
func newBindFixture(t *testing.T) *bindFixture {
	...
	if _, err := f.pool.Exec(context.Background(),
		`UPDATE user_eid_identities SET person_etsi = $2 WHERE user_id = $1`,
...
func TestASecondGoogleSignInDoesNotAskForEIDAgain(t *testing.T) {
	...
	var accounts int
	if err := f.pool.QueryRow(context.Background(),
		`SELECT count(*) FROM user_sso_identities WHERE subject = $1`, f.google.subject).Scan(&accounts); err != nil {
...
func TestASignInFindsALinkedIdentityWithoutAMembership(t *testing.T) {
	...
	if _, err := f.pool.Exec(context.Background(),
		`DELETE FROM memberships WHERE user_id = $1`, f.userID); err != nil {
