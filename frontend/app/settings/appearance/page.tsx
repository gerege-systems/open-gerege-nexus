  ...
  return (
    <div className="w-full space-y-6">
      <div className="border-b border-line pb-4 flex items-start justify-between gap-4">
        ...
        <button onClick={theme.resetTheme} className="flex items-center gap-2 px-3 py-2 text-sm border border-line rounded-lg bg-surface hover:bg-surface-hover text-muted">
      ...
      <section className="bg-surface border border-line rounded-lg p-5">
        ...
        <p className="text-sm text-muted mt-1">{t("appearance.view.languages_hint")}</p>
        <p className="text-xs text-muted mt-1 mb-4">{t("appearance.view.languages_partial")}</p>
      ...
      <section className="bg-surface border border-line rounded-lg p-5">
        ...
        <p className="text-sm text-muted mt-1 mb-4">{t("appearance.view.theme_style_hint")}</p>
        <div className="grid sm:grid-cols-2 gap-4">
          {styles.map((item) => (
            <button key={item.value} onClick={() => theme.updateTheme({ design: item.value })} className={`overflow-hidden rounded-xl border text-left transition ${theme.design === item.value ? "border-accent ring-2 ring-[color-mix(in_srgb,var(--gerege-blue)_15%,transparent)]" : "border-line hover:border-input"}`}>
              <span className="block h-12 border-b border-line" style={{ background: item.top }}>
                <span className="block w-1/3 h-full border-r border-black/10" style={{ background: item.value === "original" ? "#ffffff" : "#f7f9fc" }} />
              </span>
              <span className="flex items-center gap-3 p-4 bg-surface">
                <span className="w-3 h-8 rounded-full" style={{ background: item.accent }} />
                <span className="flex-1">
                  <strong className="block text-sm">{t(item.name)}</strong>
                  <span className="block text-xs text-muted mt-0.5">{t(item.hint)}</span>
                </span>
                {theme.design === item.value && <Check className="w-4 h-4 text-accent" />}
              </span>
            </button>
          ))}
        </div>
      ...
      <section className="bg-surface border border-line rounded-lg p-5">
        ...
        <p className="text-sm text-muted mt-1 mb-4">{t("appearance.view.color_mode_hint")}</p>
        <div className="grid sm:grid-cols-3 gap-3">
          {modes.map(({ value, label, icon: Icon }) => (
            <button key={value} onClick={() => theme.updateTheme({ mode: value })} className={`relative flex items-center gap-3 p-4 rounded-lg border text-left transition ${theme.mode === value ? "border-accent bg-accent-soft" : "border-line hover:border-input"}`}>
              <Icon className="w-5 h-5 text-accent" />
              <span className="font-medium text-sm">{t(label)}</span>
              {theme.mode === value && <Check className="absolute right-3 w-4 h-4 text-accent" />}
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="bg-surface border border-line rounded-lg p-5">
        ...
        <p className="text-sm text-muted mt-1 mb-4">{t("appearance.view.accent_hint")}</p>
        <div className="grid sm:grid-cols-2 gap-3">
          {accents.map((accent) => (
            <button key={accent.value} onClick={() => theme.updateTheme({ accent: accent.value })} className={`flex items-center gap-3 p-3 rounded-lg border text-left ${theme.accent === accent.value ? "border-accent bg-accent-soft" : "border-line hover:border-input"}`}>
              ...
              {theme.accent === accent.value && <Check className="w-4 h-4 text-accent" />}
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="bg-surface border border-line rounded-lg p-5">
        ...
        <div className="mt-4 inline-flex rounded-lg border border-line p-1 bg-surface-2">
          {densities.map(({ value, label }) => (
            <button key={value} onClick={() => theme.updateTheme({ density: value })} className={`px-4 py-2 rounded-md text-sm font-medium ${theme.density === value ? "bg-surface text-accent shadow-sm" : "text-muted"}`}>
              {t(label)}
            </button>
          ))}
        </div>
