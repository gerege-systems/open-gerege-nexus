...
export default function AccessSettingsPage(){
  ...
  if(loading)return <div className="p-8 text-muted">{t("access.message.loading")}</div>;
  return <div className="w-full space-y-6">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-accent">{t("access.view.eyebrow")}</p><h1 className="mt-1 flex items-center gap-2 text-2xl font-semibold text-foreground"><ShieldCheck className="w-6 h-6"/>{t("access.view.title")}</h1><p className="mt-1 text-sm text-muted">{t("access.view.subtitle")}</p></div><div className="inline-flex rounded-xl border border-line bg-surface p-1"><button onClick={()=>setTab("roles")} className={`px-4 py-2 rounded-lg text-sm font-semibold ${tab==="roles"?"bg-accent-soft text-accent":"text-muted"}`}><ShieldCheck className="inline w-4 h-4 mr-2"/>{t("access.view.tab_roles")}</button><button onClick={()=>setTab("members")} className={`px-4 py-2 rounded-lg text-sm font-semibold ${tab==="members"?"bg-accent-soft text-accent":"text-muted"}`}><Users className="inline w-4 h-4 mr-2"/>{t("access.view.tab_members")}</button><button onClick={()=>setTab("requests")} className={`px-4 py-2 rounded-lg text-sm font-semibold ${tab==="requests"?"bg-accent-soft text-accent":"text-muted"}`}><DoorOpen className="inline w-4 h-4 mr-2"/>{t("access.view.tab_requests")}{queue.length>0&&<span className="ml-2 rounded-full bg-accent px-2 py-0.5 text-[10px] font-semibold text-on-accent">{queue.length}</span>}</button></div></header>
    ...
    {tab==="requests"?<section className="rounded-xl border border-line bg-surface overflow-hidden">
      <div className="border-b p-5"><h2 className="font-semibold">{t("access.view.requests_title")}</h2><p className="text-sm text-muted">{t("access.view.requests_hint")}</p></div>
      {queue.length===0
        ? <p className="p-6 text-sm text-muted text-center italic">{t("access.message.no_requests")}</p>
        : <div className="divide-y">{queue.map(request=><div key={request.id} className="flex flex-wrap items-start justify-between gap-3 p-4">
            ...
            <div className="flex gap-2">
              <button disabled={saving} onClick={()=>void decide(request,true)} className="rounded-lg bg-accent px-3 py-2 text-xs font-semibold text-on-accent disabled:opacity-50"><Check className="inline w-4 h-4 mr-1"/>{t("access.action.accept")}</button>
              <button disabled={saving} onClick={()=>void decide(request,false)} className="rounded-lg border border-line px-3 py-2 text-xs font-semibold text-muted disabled:opacity-50"><X className="inline w-4 h-4 mr-1"/>{t("access.action.decline")}</button>
            </div></div>)}</div>}
    </section>:tab==="roles"?<div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="space-y-3"><div className="rounded-xl border border-line bg-surface p-3 space-y-2">{roles.map(role=><button key={role.id} onClick={()=>setSelected(role.id)} className={`w-full rounded-xl border p-3 text-left ${selected===role.id?"border-accent bg-accent-soft":"border-transparent hover:bg-surface-2"}`}><span className="flex justify-between gap-2"><strong className="text-sm">{role.name}</strong><code className="text-[10px] text-muted">{role.code}</code></span><small className="mt-1 block text-muted">{t("access.message.role_summary",{count:role.permissions.length})} · {role.active?t("base.state.active"):t("base.state.inactive")}</small></button>)}</div>
        <form onSubmit={e=>{e.preventDefault();void createRole()}} className="rounded-xl border border-line bg-surface p-4 space-y-3"><h2 className="font-semibold text-sm">{t("access.view.create_role")}</h2><input required value={newRole.code} onChange={e=>setNewRole({...newRole,code:e.target.value})} placeholder={t("access.field.code_placeholder")} className="w-full rounded-lg border px-3 py-2 text-sm"/><input required value={newRole.name} onChange={e=>setNewRole({...newRole,name:e.target.value})} placeholder={t("access.field.name_placeholder")} className="w-full rounded-lg border px-3 py-2 text-sm"/><textarea value={newRole.description} onChange={e=>setNewRole({...newRole,description:e.target.value})} placeholder={t("access.field.description_placeholder")} className="w-full rounded-lg border px-3 py-2 text-sm"/><button disabled={saving} className="w-full rounded-lg bg-accent px-3 py-2 text-sm font-semibold text-on-accent"><Plus className="inline w-4 h-4 mr-1"/>{t("access.action.create_role")}</button></form>
      </aside>
      <section className="rounded-xl border border-line bg-surface overflow-hidden">{current&&<><div className="flex items-start justify-between gap-4 border-b p-5"><div><h2 className="font-semibold text-lg">{current.name}</h2><p className="text-sm text-muted">{current.description||t("access.message.no_description")}</p></div><div className="flex gap-2">{!current.system&&<button onClick={()=>void removeRole(current)} aria-label={t("base.action.delete")} className="rounded-lg border border-red-200 p-2 text-red-600"><Trash2 className="w-4 h-4"/></button>}<button disabled={saving||current.code==="admin"} onClick={()=>void savePermissions()} className="rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-on-accent disabled:opacity-50"><Save className="inline w-4 h-4 mr-1"/>{t("base.action.save")}</button></div></div><div className="p-5 space-y-5">{current.code==="admin"&&<p className="rounded-xl bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">{t("access.message.admin_note")}</p>}{grouped.map(([app,items])=><fieldset key={app}><legend className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">{app}</legend><div className="grid gap-2 md:grid-cols-2">{items.map(p=><label key={p.code} className={`flex gap-3 rounded-xl border p-3 ${draft.includes(p.code)?"border-accent bg-accent-soft":"border-line"}`}><input type="checkbox" checked={current.code==="admin"||draft.includes(p.code)} disabled={current.code==="admin"} onChange={()=>togglePermission(p.code)} className="mt-1 h-4 w-4 accent-blue-600"/><span><strong className="block text-sm">{p.name}</strong><code className="text-[10px] text-muted">{p.code}</code><small className="block mt-1 text-muted">{p.description}</small></span></label>)}</div></fieldset>)}</div></>}</section>
    </div>:<section className="rounded-xl border border-line bg-surface overflow-hidden"><div className="border-b p-5"><h2 className="font-semibold">{t("access.view.members_title")}</h2><p className="text-sm text-muted">{t("access.view.members_hint")}</p></div><div className="divide-y">{members.map(member=><div key={member.membership_id} className="grid gap-3 p-4 lg:grid-cols-[minmax(180px,1fr)_2fr]"><div><strong className="block text-sm">{member.name}</strong><span className="text-xs text-muted">{member.email}</span>{member.is_admin&&<span className="ml-2 rounded bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-800">ADMIN</span>}</div><div className="flex flex-wrap gap-2">{roles.filter(r=>r.active).map(role=><label key={role.id} className={`rounded-lg border px-3 py-2 text-xs font-semibold cursor-pointer ${member.roles.includes(role.id)?"border-accent bg-accent-soft text-accent":"border-line"}`}><input className="sr-only" type="checkbox" checked={member.roles.includes(role.id)} onChange={()=>void toggleMemberRole(member,role.id)}/>{role.name}</label>)}</div></div>)}</div></section>}
  </div>;
}
