# Танилцуулга — ppt.open.gerege.mn
...
## ppt.open.gerege.mn дээр байршуулах
...
ssh "$HOST" 'install -m 0644 /tmp/nginx-ppt.conf.example \
                  /etc/nginx/sites-available/ppt.open.gerege.mn &&
                ln -sfn /etc/nginx/sites-available/ppt.open.gerege.mn \
                        /etc/nginx/sites-enabled/ppt.open.gerege.mn &&
                nginx -t && systemctl reload nginx'

# 3. гэрчилгээ (443 блокийг certbot өөрөө бичнэ)
ssh "$HOST" 'certbot --nginx -d ppt.open.gerege.mn --redirect'
```

Зөвхөн агуулга шинэчлэх бол 1-р алхмыг давтаад л болно — nginx reload хэрэггүй.

DNS: `ppt.open.gerege.mn` нь `*.open.gerege.mn` wildcard-аар аль хэдийн
